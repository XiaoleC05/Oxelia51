...
export class ClickHouseClientManager {
  ...
  private isEventsTableReadPathEnabled(): boolean {
    return EVENTS_TABLE_READ_PATH_ENV_KEYS.some((key) =>
      // env.mjs 对 LANGFUSE_MIGRATION_V4_ALLOW_PREVIEW_OPT_IN 的 zod 默认值是 "true"
      //（v4 target state），但这里读的是裸 process.env，默认值不会回写——
      // 未显式配置时必须与 env.mjs 的默认保持一致，否则 FTS 查询缺 enable_full_text_index。
      key === "LANGFUSE_MIGRATION_V4_ALLOW_PREVIEW_OPT_IN"
        ? (process.env[key] ?? "true") === "true"
