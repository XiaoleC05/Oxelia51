...
## 管理员体系上线（2026-08-06 23:30）

- ✅ 独立管理台 `/admin`（7 Tab：总览/反馈/用户/系统/安全/工具/告警），旧 `/project/[id]/admin` 重定向
- ✅ **安全修复**：OXELIA51_ADMIN_EMAILS 空名单 = 无人是管理员（原为全员放行）
- ✅ 生产 `OXELIA51_ADMIN_EMAILS=714085964@qq.com,postmaster@oxelia51.com`（compose 透传已应用）
  - 主管理员：714085964@qq.com（Guts，已注册，立即可用）
  - postmaster@oxelia51.com 预留（未注册，待确认邮箱可收信后注册）
- ✅ 登录页管理员模式 + 非管理员自动登出；页脚三行统一；加载动画/配色优化（commit d09bc8a 已部署）
