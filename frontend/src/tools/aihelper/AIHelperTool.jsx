...
function Edit3Icon() {
  return (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 20h9" /><path d="M16.5 3.5a2.121 2.121 0 1 1 3 3L7 19l-4 1 1-4L16.5 3.5z" /></svg>)
}
...
export default function AIHelperTool() {
  ...
  const toggleFavorite = useCallback(async (id, current) => {
    try {
      ...
      setPrompts((prev) => prev.map((p) => p.id === id ? { ...p, favorite: !current } : p))
    } catch (err) { setError(err.message) }
  }, [])
  ...
  const openEditor = useCallback((prompt) => {
    if (prompt) {
      ...
      setForm({ title: prompt.title || '', content: prompt.content || '', category: prompt.category || '通用', tags: Array.isArray(prompt.tags) ? prompt.tags.join(', ') : (prompt.tags || ''), variables: Array.isArray(prompt.variables) ? prompt.variables.join(', ') : (prompt.variables || '') })
    } else {
  ...
  const savePrompt = useCallback(async (e) => {
    ...
    setSaving(true)
    try {
      const payload = { title: form.title.trim(), content: form.content, category: form.category, tags: form.tags.split(',').map((s) => s.trim()).filter(Boolean), variables: form.variables.split(',').map((s) => s.trim()).filter(Boolean) }
      if (editingPrompt) {
        await apiProxy('aihelper', `api/prompts/${editingPrompt.id}`, { method: 'PUT', body: JSON.stringify(payload) })
      } else {
        await apiProxy('aihelper', 'api/prompts', { method: 'POST', body: JSON.stringify(payload) })
      }
      setEditorOpen(false)
  ...
  const enhancePrompt = useCallback(async () => {
    ...
    setEnhancing(true)
    try {
      const data = await apiProxy('aihelper', 'api/enhance', { method: 'POST', body: JSON.stringify({ content: form.content }) })
      if (data?.enhanced || data?.content) setForm((f) => ({ ...f, content: data.enhanced || data.content }))
  ...
  const loadSettings = useCallback(async () => {
    ...
    try {
      ...
      if (data) {
        ...
        setSettings({ api_key: s.api_key || '', api_base: s.api_base || '', model: s.model || '' })
      }
  ...
  const saveSettings = useCallback(async (e) => {
    ...
    try {
      ...
      await apiProxy('aihelper', 'api/settings', { method: 'PUT', body: JSON.stringify(body) })
    ...
    <div className="ah-shell">
          ...
          <form className="ah-editor-form" onSubmit={savePrompt}>
            <div className="ah-editor-fields">
              ...
              <div className="ah-editor-row">
                <select className="ah-select" value={form.category} onChange={(e) => setForm((f) => ({ ...f, category: e.target.value }))}>{CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}</select>
              ...
              <div className="ah-content-wrap">
                <div className="ah-content-toolbar"><span className="ah-content-label">提示词内容</span><button type="button" className="ah-enhance-btn" onClick={enhancePrompt} disabled={enhancing || !form.content.trim()}><SparklesIcon /> {enhancing ? '增强中…' : '一键增强'}</button></div>
      ...
      {!editorOpen && viewMode === 'list' && (
        <div className="ah-list">
          ...
          {promptsLoading ? (<div className="ah-loading"><div className="ah-spinner" /><p>加载提示词…</p></div>) : prompts.length === 0 ? (<p className="ah-empty-text">暂无提示词</p>) : (
            <div className="ah-prompt-list">
              {prompts.map((p) => (
                <div key={p.id} className="ah-prompt-card">
                  <div className="ah-prompt-card-body">
                    <div className="ah-prompt-card-header"><h3 className="ah-prompt-card-title">{p.title}</h3>{p.category && <span className="ah-category-tag">{p.category}</span>}</div>
                    <p className="ah-prompt-card-preview">{p.content?.slice(0, 120)}{(p.content?.length > 120) ? '…' : ''}</p>
                    <div className="ah-prompt-card-meta">{p.tags && (Array.isArray(p.tags) ? p.tags : []).length > 0 && (<span className="ah-tags">{(Array.isArray(p.tags) ? p.tags : String(p.tags).split(',').filter(Boolean).map(s => s.trim())).map((t) => (<span key={t} className="ah-tag">{t}</span>))}</span>)}{p.updated_at && (<span className="ah-time">{new Date(p.updated_at).toLocaleDateString('zh-CN')}</span>)}</div>
                  </div>
      ...
      {!editorOpen && viewMode === 'templates' && (
        <div className="ah-templates">
          <div className="ah-templates-filter">{CATEGORY_OPTIONS.map((c) => (<button key={c} className={`ah-cat-btn ${templateCategory === c ? 'ah-cat-btn--active' : ''}`} onClick={() => setTemplateCategory(c)}>{c}</button>))}</div>
          {templatesLoading ? (<div className="ah-loading"><div className="ah-spinner" /><p>加载模板…</p></div>) : templates.length === 0 ? (<p className="ah-empty-text">暂无模板</p>) : (
            <div className="ah-template-list">{templates.map((tpl) => (<div key={tpl.id} className="ah-template-card" onClick={() => applyTemplate(tpl)}><div className="ah-template-card-header"><h3 className="ah-template-card-title">{tpl.title}</h3>{tpl.category && <span className="ah-category-tag">{tpl.category}</span>}</div><p className="ah-template-card-preview">{tpl.content?.slice(0, 100)}{(tpl.content?.length > 100) ? '…' : ''}</p><span className="ah-template-hint">点击使用此模板</span></div>))}</div>
          )}
        </div>
      )}
            ...
            <form className="ah-settings-form" onSubmit={saveSettings}>
              <label className="ah-field">
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  ...
                  <button type="button" className="ah-icon-btn" onClick={() => { if (settings.api_key && settings.api_key.length > 4) navigator.clipboard.writeText(settings.api_key).catch(() => {}) }} title="复制">📋</button>
                </div>
              </label>
              <label className="ah-field">
