...
## 方向：v4（本地优先的个人 Token 记账本，P1–P4 已完成 ✅）

产品已转向**本地优先的个人 Token 记账本**（2026-08-08 设计定稿，P1–P4 已全部完成）：

- **桌面应用 v0.1.0**（`desktop/`，Tauri 2，Windows/macOS/Linux）全功能本地使用，数据存本地 SQLite；**登录/注册不再是使用前提**，仅用于跨设备同步、云托管、管理员管理
- **多设备同步**：登录账户后本地账本可上传/下载云端、多设备共用（按事件去重合并）
- **会话与项目为一等公民**，多维（纵/横/时/空）展示 Token 记录
...
> 下文 v3 描述为**云端能力**（仍可用）；桌面应用见 `desktop/` 与[下载页](https://oxelia51.com/download)。
