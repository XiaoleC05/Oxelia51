#include "pricing.h"

#include "db/postgres.h"

#include <cstdlib>
#include <libpq-fe.h>

namespace oxelia51 {

Pricing::Pricing(PostgresClient& pg) {
    loadBuiltinFallback();
    try {
        loadFromDB(pg);
    } catch (...) {
        // DB 读取失败时使用内置兜底定价
    }
}

void Pricing::loadBuiltinFallback() {
    // #25：与 analytics/deploy/migrations/009_pricing_refactor_202609.sql 保持一致
    // （DB 正常时走 loadFromDB；此兜底仅在 DB 读取失败时生效）。
    pricing_["claude-opus-5"]     = {"claude-opus-5",     "anthropic", 5.00, 25.00};
    pricing_["claude-sonnet-5"]   = {"claude-sonnet-5",   "anthropic", 2.00, 10.00};
    pricing_["gpt-4o"]            = {"gpt-4o",            "openai",    2.50, 10.00};
    pricing_["deepseek-v4-flash"] = {"deepseek-v4-flash", "deepseek",  0.44,  1.32};
}

void Pricing::loadFromDB(PostgresClient& pg) {
    const char* sql =
        "SELECT model, provider, prompt_price_usd, completion_price_usd "
        "FROM oxelia51.model_pricing";
    PgResultGuard res(pg.exec(sql));
    int n = PQntuples(res.r);
    for (int i = 0; i < n; ++i) {
        PricingInfo info;
        info.model = PQgetvalue(res.r, i, 0);
        info.provider = PQgetvalue(res.r, i, 1) ? PQgetvalue(res.r, i, 1) : "";
        info.prompt_price_usd = std::stod(PQgetvalue(res.r, i, 2));
        info.completion_price_usd = std::stod(PQgetvalue(res.r, i, 3));
        pricing_[info.model] = std::move(info);
    }
}

double Pricing::calculate(const std::string& model,
                          uint64_t prompt, uint64_t completion) const {
    auto it = pricing_.find(model);
    if (it == pricing_.end()) {
        return 0.0;
    }
    const auto& p = it->second;
    return (prompt / 1'000'000.0) * p.prompt_price_usd
         + (completion / 1'000'000.0) * p.completion_price_usd;
}

} // namespace oxelia51
