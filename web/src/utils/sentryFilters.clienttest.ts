...
describe("isNoisyHttpClientPollEvent", () => {
  ...
  describe("KEEPS genuine 5xx on real endpoints (does not mask real errors)", () => {
    ...
    it("keeps a public API 5xx (sync endpoints)", () => {
      expect(
        isNoisyHttpClientPollEvent(
          httpClientEvent("https://cloud.langfuse.com/api/sync/upload", 500),
        ),
      ).toBe(false);
      expect(
        isNoisyHttpClientPollEvent(
          httpClientEvent("https://cloud.langfuse.com/api/sync/download", 502),
        ),
      ).toBe(false);
    });
