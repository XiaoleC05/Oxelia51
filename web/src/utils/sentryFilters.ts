...
export const HTTP_CLIENT_NOISE_PATHS = [
  ...
  // The probe below is defensive/inert: it is only fetched by infra liveness
  // checks, never by the browser, so it cannot actually match a client-side
  // httpClient event. Only /api/auth/session suppresses real browser noise
  // today; the probe is listed to be safe if a browser ever starts polling it.
 ...
 *    `/api/sync/upload`, `/api/sync/download` — those are not in the noise
...
function coreMessage(value: string): string {
  ...
  //    `Failed to fetch (example.com)` -> `Failed to fetch`;
