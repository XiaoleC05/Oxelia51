...
// （原 features/search-bar/lib/rank.ts，search-bar 已删，内联保留唯一使用方）
...
): string => displayByValue?.get(value) ?? (value === "" ? "（空）" : value);
...
export function getFacetSummary(filter: UIFilter): string | null {
  if (filter.type === "categorical") {
    if (!filter.isActive) {
      ...
      return filter.options.length > 1 ? "全部" : null;
    }
    ...
    if (filter.operator === "none of") {
      ...
      if (excluded.length === 1) {
        parts.push(`排除 ${displayValue(excluded[0], filter.displayByValue)}`);
      } else if (excluded.length > 1) {
        parts.push(`排除 ${excluded.length} 个值`);
      }
    } else if (
      ...
      parts.push(`已选 ${filter.value.length} 项`);
    }
    ...
    if (filter.textFilters && filter.textFilters.length > 0) {
      if (filter.textFilters.length === 1) {
        ...
        parts.push(`${filter.textFilters.length} 个文本筛选`);
      }
    }

    return parts.length > 0 ? parts.join(" · ") : "已筛选";
  }
  ...
  return `${filter.value.length} 个条件`;
}
