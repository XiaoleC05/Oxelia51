...
import type { FilterState } from "@oxelia51/shared";
...
describe("Filter Query Encoding & Decoding (Legacy Format)", () => {
  ...
  describe("Decoding", () => {
    ...
    it("should pass through legacy root position-in-trace filter key as-is", () => {
      // 当前 fork 的 filter-builder 把 "root" 当作独立模式（根观测），
      // 不再做 root → first 的旧映射；解码器原样透传 key。
