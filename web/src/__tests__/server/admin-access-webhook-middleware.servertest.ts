...
vi.mock("@oxelia51/shared/src/server", async () => {
  const originalModule = await vi.importActual("@oxelia51/shared/src/server");
  return {
    ...originalModule,
    getTraceById: vi.fn(),
  };
...
import { getTraceById } from "@oxelia51/shared/src/server";
