...
beforeAll(async () => {
  ...
  // LANGFUSE_MIGRATION_V4_WRITE_MODE defaults to events_only in this fork, so
  // lastTraceByProject reads events_core (populated via the events_full MV) —
  // the fixture must write events, not legacy traces.
  await createEventsCh([
    createEvent({ project_id: projectId, trace_id: projectTraceId }),
    createEvent({
      project_id: siblingProjectId,
      trace_id: siblingProjectTraceId,
    }),
  ]);
});

afterAll(async () => {
  // CH rows use per-run random ids and are left behind (no events delete
  // helper exists); only the PG fixtures are cleaned up.
