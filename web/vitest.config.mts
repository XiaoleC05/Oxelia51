...
// The server suite spends more time importing the heavy @oxelia51/shared
...
const serverTestFiles = globSync("src/**/server/**/*.servertest.{ts,tsx}", {
  ...
  // vitest include patterns are forward-slash globs, so normalize or the
  // server projects match zero files on Windows.
