...
const config: StorybookConfig = {
  ...
  staticDirs: ["../public"],
  // Resolve `@oxelia51/shared` to its TypeScript source, mirroring the app's
  // own alias (next.config.mjs: webpack alias + turbopack.resolveAlias both map
  // "@oxelia51/shared" -> "./packages/shared/src"). The package's published
  ...
  viteFinal: async (viteConfig) => {
    ...
    // import like `@oxelia51/shared/src/db` would become `.../src/src/db`
    // (double `src`) and fail to resolve. The bare specifier and the
    // `@oxelia51/shared/src/...` subpaths (which the package's `exports` map
    ...
    // turbopack.resolveAlias "@oxelia51/shared" -> "./packages/shared/src").
    ...
    viteConfig.resolve.alias = [
      ...
      // `@oxelia51/shared/query`, imported transitively via the chart-library's
      ...
      {
        find: /^@oxelia51\/shared\/src\/(.*)$/,
        replacement: `${sharedSrc}/$1`,
      },
      { find: /^@oxelia51\/shared$/, replacement: sharedSrc },
      ...aliasArray,
    ];
    return viteConfig;
  },
